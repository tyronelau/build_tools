...
#
#   高级指令:
#       make test_until_die         编译并反复运行所有调试版和优化版单元测试，直到测试失败；
#                                   打印失败之前单元测试正常运行的轮数。
#       make dbg_test_until_die     编译并反复运行所有调试版的单元测试，直到测试失败；
#                                   打印失败之前单元测试正常运行的轮数。 
#       make opt_test_until_die     编译并反复运行所有调试版的单元测试，直到测试失败；
#                                   打印失败之前单元测试正常运行的轮数。
#
#   发布指令:
...
if [ $# -eq 1 ] && [ $1 = "ALL" ]; then
  ...
  python build_tools/blade3/pconfig.py $all_build && \
  echo "" && \
  ...
  # 生成新的 Makefile
  python build_tools/blade3/pconfig.py $* && \
  echo "" && \
  echo "The Makefile is generated succesfully."
fi
